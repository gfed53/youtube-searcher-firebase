...
(function () {
    ...
    function PlaylistCtrl(
        ...
        var videoItemsService = ytFirebase.services.isLoggedIn()
            ? ytVideoItemsFB
            : ytVideoItems;
        var searchHistoryService = ytFirebase.services.isLoggedIn()
            ? ytSearchHistoryFB
            : ytSearchHistory;
        ...
        vm.fbaseAuthenticate = ytFirebase.services.authenticate;
        ...
        function clearAllSearches() {
            searchHistoryService.clearAll().then((searches) => {
        ...
        function clearAllVideos() {
            videoItemsService.services.clearAllItems().then((items) => {
                vm.items = items;
            });
        }
        ...
        function sortVideos(predicate) {
            let sortObj = ytPlaylistSort.order(
                vm.videosPredicate,
                predicate,
                ytPlaylistSort.videos
            );
        ...
        function sortSearches(predicate) {
            let sortObj = ytPlaylistSort.order(
                vm.searchesPredicate,
                predicate,
                ytPlaylistSort.searches
            );
        ...
        function handleStorageSettings() {
            ytSettings.handleStorageSettings().then((res) => {
                vm.warnActive = res;
            });
        }
    }
