...
(function () {
    angular
        ...
        .factory("ytSetChannelAndNavigate", [
            "$state",
            "ytCurrentChannel",
            "ytSearchParams",
    ...
    function ytSearchYouTube(
        ...
        return (params, pageToken, direction) => {
            ...
            // Moment.js parsing
            let parsedAfter = params.after
                    ? ytDateHandler().getDate(params.after, "M/D/YYYY")
                    : undefined,
                parsedBefore = params.before
                    ? ytDateHandler().getDate(params.before, "M/D/YYYY")
                    : undefined;
    ...
    function ytVideoItems(
            ...
            if (localStorage.length) {
                for (let key in localStorage) {
                    if (key.includes("uytp")) {
                        ...
                        obj.codeName = key;
                        if (
                            ytUtilities().getIndexIfObjWithAttr(
                                items,
                                "name",
                                obj.name
                            ) === -1
        ...
        function setItem(result) {
            let dateAdded = Date.now(),
                content = result,
                itemName =
                    result.snippet.title + "-" + content.id.videoId + "-uytp",
                deferred = $q.defer();
            ...
            if (
                ...
                let errorVideoExistsTemp = ytModalGenerator().getTemp(
                    "errorVideoExistsTemp"
                );
                //Call modal
                ytModalGenerator()
                    .openModal(errorVideoExistsTemp)
                    .then(() => {
                        deferred.reject();
                    });
            }
        ...
        function clearItem(item, isWarnActive) {
            ...
            if (item.codeName) {
                //This would take place in the playlist section
                if (isWarnActive) {
                    ytModalGenerator()
                        .openModal(warnTemp)
                        .then(() => {
                ...
                //This would take place in the video section.
    ...
    function ytVideoItemsFB(
        ...
        function clearAllItems() {
            ...
            ytModalGenerator()
                ...
                .then(
                    () => {
                        var objRef = ytFirebase.services.getRefObj(
                            "savedVideos"
                        );
                        objRef.$remove().then(() => {
                            ytFirebase.services.hotSave();
    ...
    function ytSearchParams(ytTranslate) {
        ...
        function set(newParams) {
            newParams.after = newParams.after
                ? new Date(newParams.after)
                : undefined;
            newParams.before = newParams.before
                ? new Date(newParams.before)
                : undefined;
    ...
    function ytSearchHistoryFB(
                ...
                .then((name) => {
                    ...
                    if (params.name === "cancel") {
                        ...
                        params.after = params.after
                            ? params.after.getTime()
                            : null;
                        params.before = params.before
                            ? params.before.getTime()
                            : null;

                        params.nameShrt = params.name;
                        params.name = params.name + "-uyts";
                        params.date = Date.now();
                        pastSearches.$add(params).then((ref) => {
                            ytFirebase.services.hotSave();
                        });
                    } else {
                        // We call the function again because if the user doesn't provide a name for the saved search and then tries to confirm the save, we will just have the modal re-open prompting the user to try again
        ...
        function clearItem(search, isWarnActive) {
            ...
            function initClear() {
                pastSearches.$remove(search).then((ref) => {
                    ytFirebase.services.hotSave();
                });
            }
        ...
        return () => {
            ...
            function addedAfterVideos(item, filter) {
                ...
                if (filter && filter.addedAfter) {
                    if (item.dateAdded) {
                        let dateAdded = parseInt(
                                moment(
                                    ytDateHandler().getDate(
                                        item.dateAdded,
                                        "M/D/YYYY"
                                    )
                                ).format("X"),
            ...
            function addedAfterSearches(item, filter) {
                ...
                if (filter && filter.addedAfter) {
                    let dateAdded = parseInt(
                            moment(
                                ytDateHandler().getDate(item.date, "M/D/YYYY")
                            ).format("X"),
                            10
                        ),
                        after = parseInt(
    ...
    function ytModalGenerator($q, $uibModal) {
        return () => {
            ...
            let newSegTemp = {
                templateUrl:
                    "./partials/playlist/playlist-partials/modals/new-seg-modal.html",
            ...
            let searchTemp = {
                templateUrl:
                    "./partials/search/search-partials/modals/error-modal.html",
            ...
            let searchSavedTemp = {
                templateUrl:
                    "./partials/search/search-partials/modals/search-saved-modal.html",
            ...
            let videoTemp = {
                templateUrl:
                    "./partials/video/video-partials/modals/error-modal.html",
                controller: "ErrorModalController",
                controllerAs: "errorModal",
            };
            ...
            let transTemp = {
                templateUrl:
                    "./partials/search/search-partials/modals/translate-error-modal.html",
                controller: "ErrorModalController",
                controllerAs: "errorModal",
            };
            ...
            let warnTemp = {
                templateUrl:
                    "./partials/playlist/playlist-partials/modals/warn-modal.html",
            ...
            let itemRemovedTemp = {
                templateUrl:
                    "./partials/playlist/playlist-partials/modals/item-removed-modal.html",
            ...
            let dangerTemp = {
                templateUrl:
                    "./partials/playlist/playlist-partials/modals/danger-modal.html",
                controller: "DangerModalController",
                controllerAs: "dangerModal",
            };
            ...
            let errorVideoExistsTemp = {
                templateUrl:
                    "./partials/search/search-partials/modals/error-video-exists-modal.html",
                controller: "ErrorModalController",
                controllerAs: "errorModal",
            };
            ...
            let initFirebaseTemp = {
                templateUrl:
                    "./partials/playlist/playlist-partials/modals/init-firebase-modal.html",
            ...
            let storageSettingsTemp = {
                templateUrl:
                    "./partials/playlist/playlist-partials/modals/storage-settings-modal.html",
    ...
    function ytInitAPIs($http, $q, ytModalGenerator) {
        ...
        function init() {
            ...
            initKeys().then((data) => {
                ...
                this.apisObj.fBaseDB = localStorage["uyt-fBaseDB"]
                    ? JSON.parse(localStorage["uyt-fBaseDB"])
                    : this.apisObj.fBaseDB;
                deferred.resolve();
            });
        ...
        function initKeys() {
            return $http.get("/access").then((res) => {
                return res.data;
            });
        }
    ...
    function ytFirebase(
        ...
        function save() {
            let initFirebaseTemp = ytModalGenerator().getTemp(
                "initFirebaseTemp"
            );
            ytModalGenerator()
                .openModal(initFirebaseTemp)
                .then((obj) => {
                    if (obj) {
                        addCreds(obj);
                    } else {
    ...
    function ytSettings($q, ytInitAPIs, ytFirebase, ytModalGenerator) {
        let storageSettingsTemp = ytModalGenerator().getTemp(
            "storageSettingsTemp"
        );
